...
namespace libsecondlife.TestClient
{
    public class TestClient : SecondLife
    {
        ...
        public float FOLLOW_RADIUS_INNER = 5.0f;
        public float ACTION_RADIUS = 10.0f;
        public bool agentOutOfRadius = false;
        ...
        public void RegisterAllCommands(Assembly assembly)
        {
            foreach (Type t in assembly.GetTypes())
            {
                ...
                {
                    EventSender.Instance.NotifyStandardOutput(e.Message, true);
                }
            }
        }
		...
		public void DoCommand(string cmd, LLUUID fromAgentID, LLUUID imSessionID)
        {
            ...
            if (Commands.ContainsKey(firstToken))
            {
                ...
                {
                    EventSender.Instance.NotifyStandardOutput(response, false);
        ...
        private void AgentDataUpdateHandler(Packet packet, Simulator sim)
        {
            ...
            {
                EventSender.Instance.NotifyStandardOutput("Got the group ID for " + sim.Client.ToString() + ", requesting group members...", false);
        ...
        {
            EventSender.Instance.NotifyStandardOutput("<" + this.ToString() + "> " + level.ToString() + ": " + message, false);
        }

        private void GroupMembersHandler(Dictionary<LLUUID, GroupMember> members)
        {
            EventSender.Instance.NotifyStandardOutput("Got " + members.Count + " group members.", false);
            ...
            MainAvatar.InstantMessageOnline offline, byte[] binaryBucket, Simulator simulator)
        {
            if (MasterKey != LLUUID.Zero)
            {
                if (fromAgentID != MasterKey)
                {
                    ...
                    EventSender.Instance.NotifyStandardOutput("<IM ("+dialog+")>" + fromAgentName + " (not master): " + message + "@"  + regionID.ToString() + ":" + position.ToString(), false);
                    return;
                }
            }
            else
            {
                if (GroupMembers != null && !GroupMembers.ContainsKey(fromAgentID))
                {
                    ...
                    EventSender.Instance.NotifyStandardOutput("<IM>" + fromAgentName + " (not in group): " + message + "@" + regionID.ToString() + ":" + position.ToString(), false);
                    return;
                }
            }

            EventSender.Instance.NotifyStandardOutput("<IM>" + fromAgentName + ": " + message, false);

            if (dialog == MainAvatar.InstantMessageDialog.RequestTeleport)
            {
                EventSender.Instance.NotifyStandardOutput("Accepting teleport lure.", false);
            ...
            else
            {
                ...
                {
                    EventSender.Instance.NotifyStandardOutput("Accepting inventory offer.", false);
