...
{
    class UtilityLessVariable : Base_utility
    {
        public UtilityLessVariable() { }
        public override List<Strategy> GetStrategies(Dictionary<string, ActorSubnet> kb, Subnet subnet, string state)
        {
            ...
            if (state != "*" && state != "self") { return af.LessVariable(kb, subnet, Convert.ToInt32(state)); }
               
            for (int i = 0; i < subnet.Net.GetOutcomeCount(subnet.Name + '_' + Constants.PREMISE); i++)
            {
                strategies.AddRange( af.LessVariable(kb, subnet, Convert.ToInt32(i)));
            }
